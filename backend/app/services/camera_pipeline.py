...
def _scan_tables(db: Session) -> None:
    ...
    for table in tables:
        ...
        # Custom model: an unmatched table (blocked camera, weak frame) is
        # "no reading", never a clean verdict. COCO fallback: no "clean" class
        # exists, so an empty ROI yields no detections and absence IS the
        # clean signal — without it CLEANING could never advance to AVAILABLE.
        detection = smooth_table_state(matched, treat_none_as_clean=is_using_fallback_model())
