...
same service functions the UI uses — so every change the assistant makes goes
...
def _fallback_reply(db: Session, message: str) -> str:
    text = message.lower()
    offline = " (AI model is offline — data-only answer.)"
    ...
    return (
        ...
        "Current floor snapshot — " + "; ".join(lines) + "."
    )
...
def chat(db: Session, user: User, messages: list[dict]) -> tuple[str, list[ChatAction]]:
    system_prompt = (
        ...
        "The live floor snapshot below is the ONLY source of truth for table statuses — "
    ...
    try:
        with httpx.Client(timeout=60.0) as client:
            ...
            # Ran out of rounds or empty reply — summarize what happened.
            if actions:
                return _summarize_actions(actions), actions
            return _fallback_reply(db, last_user_message), actions
    except (httpx.HTTPError, OSError) as exc:
        logger.info("Ollama chat unavailable (%s) — using deterministic fallback", exc)
        return _fallback_reply(db, last_user_message), actions
